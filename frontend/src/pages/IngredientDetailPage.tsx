  ...
  return (
    <section className="rounded-3xl border border-orange-100 bg-white/90 p-5 shadow-sm backdrop-blur">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-medium uppercase tracking-[0.2em] text-orange-700">Detalle de ingrediente</p>
          <h1 className="text-3xl font-semibold text-orange-950">{ingrediente?.nombre ?? "Ingrediente"}</h1>
        </div>
        <Link to="/ingredientes" className="rounded-full border border-orange-200 bg-orange-50 px-4 py-2 text-sm font-medium text-orange-900">
          Volver a ingredientes
        </Link>
      </div>

      {detailQuery.isLoading ? <p className="text-sm text-orange-800">Cargando detalle...</p> : null}
      {detailQuery.isError ? <p className="rounded-xl bg-red-100 px-4 py-3 text-sm text-red-700">No se pudo cargar el ingrediente.</p> : null}

      {ingrediente ? (
        <div className="grid gap-5 lg:grid-cols-[1fr,1fr]">
          <div className="space-y-4">
            <div className="rounded-2xl border border-orange-100 bg-orange-50/60 p-4">
              <p className="text-sm text-slate-700">{ingrediente.descripcion || "Sin descripción"}</p>
              <div className="mt-3 flex flex-wrap gap-2 text-sm">
                <span className="rounded-full bg-white px-3 py-1 font-medium text-orange-900">{ingrediente.activo ? "Activo" : "Inactivo"}</span>
                <span className="rounded-full bg-white px-3 py-1 font-medium text-orange-900">{ingrediente.unidad_medida}</span>
                <span className="rounded-full bg-white px-3 py-1 font-medium text-orange-900">{ingrediente.es_alergeno ? "Alérgeno" : "No alérgeno"}</span>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-orange-100 bg-white p-4">
            <h2 className="text-lg font-semibold text-orange-950">Productos relacionados</h2>
            {ingrediente.productos_relacionados.length > 0 ? (
              ...
              <p className="mt-3 text-sm text-slate-600">Este ingrediente aún no está relacionado con productos.</p>
            )}
          </div>
        </div>
      ) : null}
