...
class TestSeedRequiredData:
    ...
    def test_crea_cinco_estados_con_es_terminal(self, seed_session: Session):
        ...
        assert set(estados) == {"PENDIENTE", "CONFIRMADO", "EN_PREP", "ENTREGADO", "CANCELADO"}
        # Solo ENTREGADO y CANCELADO son terminales (consigna §3.4)
    ...
    def test_es_idempotente(self, seed_session: Session):
        ...
        assert len(seed_session.exec(select(EstadoPedido)).all()) == 5
